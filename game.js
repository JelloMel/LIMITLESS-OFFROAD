...
(() => {
  ...
  function showIntro(){stopCheer();state='splash';keys.clear();clearTouch();resetCamera();$('intro').hidden=false;$('intro').classList.remove('flying');$('flyoverCaption').hidden=true;$('menu').hidden=true;$('finishMenu').hidden=true;$('pauseMenu').hidden=true;$('countdown').hidden=true;$('pauseButton').disabled=true;$('resetButton').disabled=true;$('enterCircuit').focus();}
  function enterCircuit(){
    if(state!=='splash')return;
    if(window.matchMedia('(prefers-reduced-motion: reduce)').matches){home();return;}
    state='flyover';introTime=0;keys.clear();clearTouch();$('intro').classList.add('flying');$('flyoverCaption').hidden=false;
  }
  ...
  function start(){winnerMoment=null;$('winnerPhoto').hidden=true;$('winnerImage').removeAttribute('src');unlockAudio();stopCheer();leaderId=null;pendingLeader=null;leaderHold=0;cheerTime=0;$('crowdCall').hidden=true;resetCamera();$('intro').hidden=true;rider=($('playerName').value.trim().slice(0,8)||'RIDER').toUpperCase();$('playerName').value=rider;try{localStorage.setItem('dustline-rider',rider);}catch{}$('riderName').textContent=rider;$('best').textContent=record()?format(record()):'—';elapsed=0;lapStart=0;lap=1;splits=[];progress=0;lastSafe=0;wheelieSeconds=0;particles=[];resetBike();resetRivals();keys.clear();clearTouch();state='countdown';count=3;$('menu').hidden=true;$('finishMenu').hidden=true;$('pauseMenu').hidden=true;$('countdown').hidden=false;$('countdown').textContent='3';$('pauseButton').disabled=false;$('resetButton').disabled=true;canvas.focus();updateHUD();}
  function home(){stopCheer();resetCamera();$('intro').hidden=true;state='menu';keys.clear();clearTouch();$('menu').hidden=false;$('pauseMenu').hidden=true;$('finishMenu').hidden=true;$('countdown').hidden=true;$('pauseButton').disabled=true;$('resetButton').disabled=true;$('playerName').focus();}
  let resumeState='race';
  function pause(){if(state==='race'||state==='countdown'){resumeState=state;stopCheer();state='paused';keys.clear();clearTouch();$('pauseMenu').hidden=false;$('countdown').hidden=true;$('resumeButton').focus();}else if(state==='paused'){unlockAudio();state=resumeState;$('pauseMenu').hidden=true;$('countdown').hidden=state!=='countdown';canvas.focus();}}
  ...
  window.addEventListener('blur',()=>{keys.clear();clearTouch();if(state==='race'||state==='countdown')pause();});
  document.addEventListener('visibilitychange',()=>{if(document.hidden){keys.clear();clearTouch();if(state==='race'||state==='countdown')pause();}});
  ...
  function clearTouch(){touchPointers.clear();document.querySelectorAll('[data-key]').forEach(b=>{keys.delete(b.dataset.key);b.classList.remove('active');});}
  document.querySelectorAll('[data-key]').forEach(button=>{
    const release=e=>{touchPointers.delete(e.pointerId);const held=[...touchPointers.values()].includes(button.dataset.key);if(!held){keys.delete(button.dataset.key);button.classList.remove('active');}};
    button.addEventListener('pointerdown',e=>{e.preventDefault();if(state!=='race'&&state!=='countdown')return;button.setPointerCapture(e.pointerId);touchPointers.set(e.pointerId,button.dataset.key);keys.add(button.dataset.key);button.classList.add('active');});
  ...
  $('fullscreenButton').onclick=async()=>{try{if(document.fullscreenElement){await document.exitFullscreen();return;}await document.documentElement.requestFullscreen();try{await screen.orientation.lock('landscape');}catch{}}catch{notify('TURN YOUR PHONE SIDEWAYS TO PLAY');}};
  ...
  const portrait=window.matchMedia('(orientation: portrait)');portrait.addEventListener('change',e=>{if(e.matches&&window.matchMedia('(pointer: coarse)').matches&&(state==='race'||state==='countdown'))pause();});
